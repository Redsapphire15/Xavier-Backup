...
class IK_IROC():
    ...
    def angle_value(self):
       ...
       # print(np.arccos(self.pz/(2*k)))
        ...
        if self.enc_data[0] <= 4 and self.enc_data[1] <=4 and self.enc_data[2] <= 4 and self.input == 1:
            self.listen_to_tanish = True
            self.arm_goal_coming_from_tanish = True
        if (abs(self.enc_data[0] - base) <= 3 and abs(self.enc_data[1] - shoulder) <= 3 and abs(self.enc_data[2] - elbow) <= 5) or time.time()- self.start_time > 180:
